...
#' readJsonFiles
#'
#' @description a function reads in the data from json files within a directory
#'
#' @param dataPath a directory containing the json files
#' @param nEnd number of characters in the files name
#'
#' @return returns a data set by combining all the existing data
#'
#' @import magrittr
#' @import dplyr
#' @import tidyr
...
readJsonFiles <- function(dataPath, nEnd){
  ...
  for (i in list.of.files){
    #  The name of the protein should be changed here
    json.file <- paste(dataPath, i,

                       sep= "")

    raw.data <- fromJSON(json.file)

    raw.data %<>% mutate(data.set.name =
                           substr(i, start = 1,
                                  stop = nchar(i)-nEnd1))%>%
...
TransformData <- function(InputData, nameList, separator, numerName){
  ...
  for(i in numerName) {
    ...
    #   as.numeric(str_extract(Output[as.character(i)], "[0-9,.]+"))
  }
...
CorrectBlinking <- function(InputData, x.distance, y.distance){
  output <- InputData %>%
    ...
    mutate(setx = ifelse(pick ==1 & abs(lead(X, n=1)-lag(X, n=1)) < x.distance &
                           abs(lead(Y, n=1)-lag(Y, n=1)) < y.distance,
                         (lead(X, n=1)+ lag(X, n=1))/2,0),
           sety= ifelse(pick ==1 & abs(lead(X, n=1)-lag(X, n=1)) < x.distance &
                          abs(lead(Y, n=1)-lag(Y, n=1)) < y.distance,
...
ArrangeData <- function(RawDataSet, DetectedTrajectories){
  ...
  # select(analysis.id, data.set.name, sub.data.set.name, trajectory.id, frame.number, X, Y,
  #        intensity, protein, signal.type)
  #
...
detectTrajectories <- function(filteredDataSet, dxMax , dyMax ){
  ...
  for(i in filteredDataSet %>%
    ...
    print(paste("Processing: ",round(100* j/ (4* length(filteredDataSet %>%
                                                          "$"(data.set.name) %>%
                                                          unique()))) , " %", sep=""))
...
detectStuck <- function(filteredDataSet, dxMax , dyMax ){
  ...
  for(i in filteredDataSet %>%
    ...
    print(paste("Processing: ", round(100*j/ (length(filteredDataSet %>%
                                                       "$"(sub.data.set.name) %>%
                                                       unique())*4/3)) , " %", sep=""))
    ...
    #  Saparate the data sets into raw ans filtered
...
AddDurationFilter <- function(data.set, minFrameDuration){
  ...
  data.set %<>%
    ...
    mutate(duration.filter = ifelse(duration>= minFrameDuration, TRUE, FALSE)) %>%
    select(-duration) %>% ungroup()
...
AddVisualInspection <- function(data.set, TrajsToInspect){
  ...
  visual.inpsection.filter <- data.set %>% filter(signal.type != "stuck", duration.filter) %>%
    group_by(sub.data.set.name, trajectory.id) %>%
    ...
    mutate(visual.inspection = TRUE) %>% select(-x.range) %>% ungroup()

  data.set <- left_join(data.set, visual.inpsection.filter) %>%
    mutate(visual.inspection = ifelse(is.na(visual.inspection), FALSE, visual.inspection))
...
{
  data.set %<>% arrange(data.set.name, sub.data.set.name,frame.number) %>%
    unite(trajectory.unique.id, sub.data.set.name, trajectory.id, remove = FALSE) %>%
...
AddRotatedCoord <- function(data.set){
  fit.on.trajectory <- data.set %>%
    filter(duration.filter, visual.inspection, signal.type!="stuck" ,!is.na(sub.data.set.name)) %>%
    group_by(sub.data.set.name) %>%
    do(tidy(lm(Y ~ X, data = .))) %>%
    mutate(fit.slope = estimate[term == "X"],
           fit.intercept = estimate[term == "(Intercept)"]) %>%
    filter(term == "X") %>%
    select(sub.data.set.name, fit.slope, fit.intercept) %>% ungroup()
  ...
  anchor.point <- data.set%>%
    filter(duration.filter, visual.inspection, signal.type!="stuck" ,!is.na(sub.data.set.name)) %>%
    group_by(sub.data.set.name) %>%
    summarise(x.translate = mean(X)-mean(r.X), y.translate = mean(Y)- mean(r.Y))
...
AddStuckFilter <- function(data.set, gauge){
  stuck.filters <- data.set %>% filter(duration.filter, signal.type == "stuck") %>%
    group_by(data.set.name, trajectory.unique.id) %>%
    summarise(sd.x = sd(X), sd.y = sd(Y), range.x = max(X)-min(X), range.y =max(Y) - min(Y)) %>%
    ungroup() %>%
    group_by(data.set.name) %>%
    summarise(sd.x.stuck.filter = mean(sd.x) + if_else(is.na(gauge*sd(sd.x)), (gauge/10)*mean(sd.x), gauge*sd(sd.x)),
              sd.y.stuck.filter = mean(sd.y) + if_else(is.na(gauge*sd(sd.y)), (gauge/10)*mean(sd.y), gauge*sd(sd.y)),
              range.x.stuck.filter = mean(range.x) + if_else(is.na(gauge*sd(range.x)), (gauge/10)*mean(range.x), gauge*sd(range.x)),
              range.y.stuck.filter = mean(range.y) + if_else(is.na(gauge*sd(range.y)), (gauge/10)*mean(range.y), gauge*sd(range.y))) %>%
    ungroup()


  data.set %<>% group_by(trajectory.unique.id) %>%
    mutate(sd.x = sd(r.X), sd.y = sd(r.Y), range.x = max(r.X)-min(r.X), range.y =max(r.Y) - min(r.Y)) %>%
    left_join(., stuck.filters) %>%
    mutate(stuck.filter = ifelse(sd.x > sd.x.stuck.filter| sd.y > sd.y.stuck.filter, TRUE, FALSE)) %>%
    mutate(stuck.filter = ifelse(range.x > range.x.stuck.filter| range.y > range.y.stuck.filter,
                                 stuck.filter, FALSE)) %>%
    mutate(stuck.filter = ifelse (is.na(sd.x.stuck.filter), TRUE, stuck.filter)) %>%
    ungroup()
...
AddFlankFilter <- function(data.set, gauge){
  ...
  on.dna.filters <- data.set %>%
    ...
    summarise(range.y= max(r.Y) - min(r.Y), sd.y= sd(r.Y), mid.y = mean(r.Y)) %>%
    ungroup() %>%
    group_by(sub.data.set.name) %>%
    summarise(range.y.flank.filter = mean(range.y) + if_else(is.na(gauge*sd(range.y)), (gauge/10)*mean(range.y), gauge*sd(range.y)),
              sd.y.flank.filter = mean(sd.y)+ if_else(is.na(gauge*sd(sd.y)), (gauge/10)*mean(sd.y), gauge*sd(sd.y)),
              y.offset.filter = mean(mid.y)) %>% ungroup()
  ...
  data.set %<>% group_by(trajectory.unique.id) %>%
    ...
    mutate(on.dna.filter = ifelse(sd.y < sd.y.flank.filter &
                                    ...
                                    abs(offset-y.offset.filter) < range.y.flank.filter/2 , TRUE, FALSE)) %>% ungroup()
...
AddIntensityFilter <- function(data.set, gauge){
  intensity.filters <- data.set %>% filter(duration.filter, visual.inspection)%>%
    group_by(data.set.name) %>%
    summarise(intensity.limit = mean(intensity) + gauge*sd(intensity)) %>% ungroup()

  data.set %<>% group_by(trajectory.unique.id) %>%
    mutate(mean.intensity = mean(intensity)) %>%
    left_join(., intensity.filters) %>%
    mutate(intensity.filter = ifelse(mean.intensity < intensity.limit, TRUE, FALSE)) %>% ungroup()
  return(data.set)
}
...
ArrangeFilters <- function(data.set){
  ...
  data.set %<>% mutate(detected.data = ifelse(duration.filter & stuck.filter & on.dna.filter & intensity.filter,
                                              "Noise-excluded data", "detected"),
                       detected.data = ifelse (!duration.filter,
                                               "Short-lived noise", detected.data),
                       detected.data = ifelse (!intensity.filter & duration.filter,
                                               "High/low intensty noise", detected.data),
                       detected.data = ifelse (!stuck.filter & intensity.filter & duration.filter,
                                               "Surface-bound emitters", detected.data),
                       detected.data = ifelse (!on.dna.filter & stuck.filter & intensity.filter & duration.filter,
                                               "Non-linear movements", detected.data))
...
AddBindingLifetime <- function(data.set, protein.filter, experimental.condition) {
  data.set %>%
    filter(protein %in% protein.filter,
           detected.data == "Noise excluded") %>%
    group_by(trajectory.unique.id) %>%
    mutate(step=lead(r.X, n=1)- r.X) %>%
    group_by(protein, trajectory.unique.id, across(all_of(experimental.condition))) %>%
    summarise(binding.lifetime = mean((max(frame.number) - min(frame.number))* unique(frame.interval)/1000),
              scanning.coverage = mean((max(r.X)- min(r.X))/0.34),
              bases.scanned = mean(max(cumsum(abs(step)), na.rm = T)/0.34),
              stepping.speed = mean(mean(abs(step), na.rm =T)/frame.interval))
